module Gameverif.Ecsy.Concrete where
...
import qualified Gameverif.Viper.Concrete as VC
...
newtype AnnStmtSeqF p v a = AnnStmtSeqF { unAnnStmtSeqF :: AnnF p (StmtSeqF (VC.AnnExp p) (AnnExp p)) v a }
  deriving stock (Show)
  deriving newtype (Eq, Functor)
...
instance WrappedF (AnnStmtSeqF p v) where
  type WrappedFuncF (AnnStmtSeqF p v) = StmtSeqF (VC.AnnExp p) (AnnExp p) v
...
mkAnnStmtSeq :: p -> StmtSeqF (VC.AnnExp p) (AnnExp p) v (AnnStmtSeq p v) -> AnnStmtSeq p v
mkAnnStmtSeq p s = AnnStmtSeq (AnnStmtSeqF (AnnF (Located p s)))

forgetAnnStmtSeq :: AnnStmtSeq p v -> StmtSeq v
forgetAnnStmtSeq (AnnStmtSeq (AnnStmtSeqF (AnnF (Located _ s)))) = StmtSeq (stmtSeqMapBoth VC.forgetAnnExp forgetAnnExp (second forgetAnnStmtSeq s))
